...
public class ReadInitStep implements IReaderStep {
    ...
    public void read(SchematicReaderContext ctx) throws SchematicReadException {
        int _version = ctx.root.getInt(NBT_VERSION,
                        (e) -> new SchematicReadException("Failed to read version", e));

        SpongeVersion version = switch (_version) {
            case 1 -> SpongeVersion.V1;
            case 2 -> SpongeVersion.V2;
            case 3 -> SpongeVersion.V3;
            default -> throw new SchematicReadException("Invalid version");
        };
        ctx.builder.setVersion(version);

        short w = ctx.root.getShort(NBT_WIDTH, (e) -> new SchematicReadException("Failed to read width", e));
        short h = ctx.root.getShort(NBT_HEIGHT, (e) -> new SchematicReadException("Failed to read height", e));
        short l = ctx.root.getShort(NBT_LENGTH, (e) -> new SchematicReadException("Failed to read length", e));
