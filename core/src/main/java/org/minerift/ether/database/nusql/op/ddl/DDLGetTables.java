...
public class DDLGetTables {
    ...
    public static Set<String> getDatabaseTables(NuSQLAccess access) {
        ...
        Set<String> tableNames = switch(access.dialect()) {
            ...
            case H2 -> {
                final Field<String> TABLE_NAME = field("table_name", TABLE_NAME_TYPE);
                yield dsl.select(TABLE_NAME).from(table("INFORMATION_SCHEMA.tables")).where(field("table_schema").eq("PUBLIC")).fetchSet(TABLE_NAME);
            }
        };
