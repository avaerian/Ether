...
public class SQLAccess extends DatabaseAccess implements AutoCloseable {
    ...
    @Override
    public <MO, PK> SQLResult<MO> selectById(Model<MO, PK> model, PK id) {
        Result<Record> result = db().selectByIdQuery.createExecutableQuery(
                this, model, NamedBindValues.of(model.getPrimaryKey(), id)).fetch();
        return new SQLResult<>(model, result);
    }

    @Override
    public <MO, PK> SQLResult<MO> selectAll(Model<MO, PK> model) {
    ...
    public <MO, PK> Stream<PK> selectAllIds(Model<MO, PK> model) {
        ...
        SQLResult<MO> result = new SQLResult<>(model, jooqResult);
