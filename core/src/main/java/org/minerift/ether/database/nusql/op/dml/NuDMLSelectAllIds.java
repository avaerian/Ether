...
public class NuDMLSelectAllIds extends NuDMLSelectOp {

    public NuDMLSelectAllIds(SQLDatabaseCreationContext dbCtx) {
        super(dbCtx);
    }

    @Override
    protected <OBJ, PK> RawModelQuery createQueryForModel(SQLDatabaseCreationContext ctx, Model<OBJ, PK> model) {
        String sql = ctx.dsl().select(asJooqField(model.getPrimaryKey())).from(asJooqTable(model)).getSQL();
