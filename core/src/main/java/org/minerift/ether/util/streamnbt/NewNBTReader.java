package org.minerift.ether.util.streamnbt;
...
import org.minerift.ether.util.nunbt.tags.container.RootEntryTag;
import org.minerift.ether.util.streamnbt.token.StatelessToken;
import org.minerift.ether.util.streamnbt.token.StatelessTokenSchema;
import org.minerift.ether.util.streamnbt.token.Token;
import org.minerift.ether.util.streamnbt.token.dyn.CompoundChildrenContent;
import org.minerift.ether.util.streamnbt.token.dyn.ExpandingTokenGroup;
import org.minerift.ether.util.streamnbt.token.dyn.ListChildrenContent;
    ...
    System.out.println(tokens); // debug
        switch(token) {
            ...
            case Token.ListChildTagIdToken tok -> {
                ...
                TagType childType = TagType.lookup(id);
            ...
            case ListChildrenContent tok -> {
                if(!tok.isReady()) {
                    ...
                    NBT.TagType depType = NBT.TagType.getType(tag.getChildType().getId()); // for compatibility
                    tok.setChildType(depType);
